...
function RoleTimelineItem({ isLatest, showLine }: RoleTimelineItemProps) {
  return (
    <div className="relative w-6 flex-shrink-0 pt-2">
      {showLine ? (
        <div
          className="absolute left-1/2 top-4 bottom-0 w-px -translate-x-1/2 bg-white/10"
          aria-hidden="true"
        />
      ) : null}
      <span
        className={[
          "absolute left-1/2 top-2 h-2 w-2 -translate-x-1/2 rounded-full ring-2 ring-white/10",
          isLatest ? "bg-pink-300/90" : "bg-white/25",
        ].join(" ")}
        aria-hidden="true"
      />
    </div>
      ...
      <header className="flex items-start justify-between gap-6">
        ...
        <div className="text-right">
          ...
          {company.totalDuration ? (
            <p className="mt-1 text-xs text-pink-200/90">{company.totalDuration}</p>
          ) : null}
        </div>
      </header>
        ...
        {orderedRoles.map((role: ExperienceRole, idx: number) => {
          ...
          return (
            <div key={role.role + role.dates} className="flex gap-4">
              ...
              <div className={`flex-1 ${hasDivider ? "border-b border-white/10 pb-6" : ""}`}>
                <div className="flex items-start justify-between gap-6">
                  ...
                  <p className="text-right font-jetbrains-mono text-xs text-slate-500">
                    {role.dates}
                  </p>
                </div>

                <p className="mt-3 text-sm leading-relaxed text-slate-400">{role.summary}</p>
